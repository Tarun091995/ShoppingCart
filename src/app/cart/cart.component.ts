...
export class CartComponent {
    ...
    quantity : number;
    cartForm: FormGroup;
    constructor(public appService: ApplicationService, public router: Router, public fb: FormBuilder) {
    ...
    addProducts() {
        if (this.appService.addedProducts.length > 0 ) {
            this.appService.addedProducts.forEach((product) => {
                let p = this.fb.group({
                    id: product.id,
                    name: product.name,
                    price: product.price,
    ...
    removeProduct(i) {      
        ...
        this.appService.addedProducts.splice(i, 1);
    }
    ...
    get product () {
        return this.cartForm.controls.products as FormArray;
    }
